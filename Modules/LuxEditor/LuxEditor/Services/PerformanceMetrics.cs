...
namespace LuxEditor.Services
{
    ...
    public class MetricSample
    {
        ...
        public string Phase { get; set; } = string.Empty; // Kept for backward compatibility
    ...
    public class ScopedTimer : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            var sample = new MetricSample
            {
                ...
                Phase = "", // Empty for new-style measurements
    ...
    public class PerformanceMetrics
    {
        ...
        /// <summary>
        /// Backward-compatible method for old-style measurements.
        /// Maps to new Measure() method with operation:phase naming.
        /// </summary>
        public ScopedTimer MeasureOperation(string operationName, string phase = "default", Dictionary<string, object>? metadata = null)
        {
            // Convert old-style "Operation" + "Phase" to new-style "Operation:Phase"
            string fullName = string.IsNullOrEmpty(phase) || phase == "default"
                ? operationName
                : $"{operationName}:{phase}";
            return new ScopedTimer(this, fullName, metadata);
        }
        ...
        public void RecordSample(MetricSample sample)
        {
            ...
            // Use OperationName directly (new style) or combine with Phase (old style)
            string key = string.IsNullOrEmpty(sample.Phase)
                ? sample.OperationName
                : $"{sample.OperationName}:{sample.Phase}";
        ...
        private void PrintSampleToConsole(MetricSample sample)
        {
            ...
            string opName = string.IsNullOrEmpty(sample.Phase)
                ? sample.OperationName
                : $"{sample.OperationName}:{sample.Phase}";

            LogToConsole($"[{timestamp}] ", ConsoleColor.DarkGray, false);
            LogToConsole($"[PERF] ", ConsoleColor.Magenta, false);
            LogToConsole($"{opName,-25} ", ConsoleColor.White, false);
        ...
        public Dictionary<string, OperationStats> GetStatistics()
        {
            ...
            foreach (var kvp in _operationTimes)
            {
                ...
                var samples = _samples.Where(s =>
                {
                    string key = string.IsNullOrEmpty(s.Phase) ? s.OperationName : $"{s.OperationName}:{s.Phase}";
                    return key == kvp.Key;
                }).ToList();
        ...
        private void ExportToCsv(string path)
        {
            ...
            foreach (var sample in _samples.OrderBy(s => s.Timestamp))
            {
                string opName = string.IsNullOrEmpty(sample.Phase)
                    ? sample.OperationName
                    : $"{sample.OperationName}:{sample.Phase}";
                lines.Add($"{sample.Timestamp:O},{opName},{sample.DurationMs:F3},{sample.MemoryDelta},{sample.Gen0Collections},{sample.Gen1Collections},{sample.Gen2Collections}");
            }
